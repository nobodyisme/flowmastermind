...
var Jobs = (function () {
    ...
    JobsView.prototype.taskTitle = function(state) {
        ...
        } else if (state['type'] == 'fix_eblobs_job') {
            title = 'Запуск eblob_kit для группы ' + state['group_id'];
    ...
    JobsView.prototype.renderRemoveGroupsetFromCouple = function(task_state, task_maintitle, task_subtitle, task_additional_data) {
        var title = 'Удаление групсета ' + task_state['groupset'] + ' из записи капла ' + task_state['couple'] + ' в базе '
            '(тип ' + task_state['groupset_key'] + ')';
        task_maintitle.html(title);
        task_maintitle.attr('title', title);
        task_subtitle.text('сохранение в монгу информации о групсете');
